...
3. Edit [config/combi.conf](config/combi.conf) to add/enable/disable features. Edit [config/combi.keymap](config/combi.keymap) to change the keymap.
...
You can add the combi zmk config repository as a module by modifying your `config/west.yml` file.
...
After making this change, add a copy of [the configuration file](config/combi.conf) and [the keymap](config/combi.keymap) to the `config` folder that is already in your repo.
...
#### Forget the Bluetooth connection on both the combi and the host, then try re-pairing.
